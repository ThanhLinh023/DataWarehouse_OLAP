...
SELECT [Measures].[Number of Games] ON COLUMNS,
    HEAD( ORDER([Dim Team].[teamName].[teamName].MEMBERS,
            [Measures].[Number of Games],
            DESC), 5) ON ROWS
FROM (
    SELECT [Dim Team].[teamName].[teamName].MEMBERS ON COLUMNS,
			[Measures].[Number of Games] ON ROWS
    FROM (
        SELECT [Dim Team].[teamKey].CHILDREN ON COLUMNS
        FROM [NBAStatsDW])
);
-- Câu 2: Tổng số lần ghi điểm (FGM) của tất cả các trận đấu theo từng năm và từng tháng trong năm
SELECT [Measures].[FGM] ON COLUMNS,
		[dim Time].[year].[Year].MEMBERS * [dim Time].[month].[Month].MEMBERS ON ROWS
FROM [NBAStatsDW];
...
SELECT
    FILTER(
        [Dim Team].[teamName].[teamName].MEMBERS,
...
FROM [NBAStatsDW]
WHERE ([Dim Game].[2ndTeam].[2ndTeam].&[GSW]);
